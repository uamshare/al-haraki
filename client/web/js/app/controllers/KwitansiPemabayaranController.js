...
define(['app'], function(app) {
    var injectParams = ['$CONST_VAR', 'helperService', '$scope', 'toastr', '$location', '$routeParams', '$http', '$log', 'ngDialog', 'cfpLoadingBar', '$timeout', 'uiGridConstants', 'kwitansiPemabayaranService', 'SiswaRombelService', 'TagihanInfoService', 'authService', 'KelasService', '$filter'];
    var KwitansiPemabayaranController = function($CONST_VAR, helperService, $scope, toastr, $location, $routeParams, $http, $log, ngDialog, cfpLoadingBar, $timeout, uiGridConstants, kwitansiPemabayaranService, SiswaRombelService, TagihanInfoService, authService, KelasService, $filter) {
        ...
        var indexController = function() {
            ...
            var columnActionTpl = '<div class="col-action">' + '<a href="" ng-click="grid.appScope.onPrintClick(row.entity)" >' + '<span class="badge bg-orange"><i class="fa fa-print"></i></span>' + '</a>&nbsp;' + '<a href="" ng-click="grid.appScope.onEditClick($event, row.entity)" >' + '<span class="badge bg-blue"><i class="fa fa-edit"></i></span>' + '</a>&nbsp;' + '<a href="" ng-click="grid.appScope.onDeleteClick(row.entity)" >' + '<span class="badge bg-red"><i class="fa fa-trash"></i></span>' + '</a>' + '</div>';
            ...
            function get(paramdata) {
                ...
                cfpLoadingBar.start();
                $resourceApi.get(paramdata).then(function(result) {
                    if (result.success) {
            ...
            function deleteData($no) {
                cfpLoadingBar.start();
                $resourceApi.delete($no).then(function(result) {
                    if (result.success) {
            ...
            $scope.onEditClick = function(event, rowdata) {
                helperService.navigateTo(event, "/keuangan/kwitansi-pembayaran/edit/" + rowdata.no_kwitansi, rowdata.no_kwitansi);
            }
            ...
            var exportTo = {
                xls: function(gridApi) {
                    function download(id) {
                        ...
                        // console.log(table);
                        var ctx = {
                            worksheet: name || 'Rekap_Pembayaran ' + postfix,
        ...
        function printClick(rowdata) {
            ...
            $resourceApi.getDetail({
                ...
                if (result.success) {
                    ...
                    $scope.tanggal = date.getDate() + ' ' + helperService.getMonthName(date.getMonth()) + ' ' + date.getFullYear();
                    ...
                    if (blnTagihan && blnTagihan.length == 1) {
                        ...
                        $scope.bulanTagihan = helperService.getMonthNameShort(parseInt(blnTagihan[0]) - 1) + ' s/d ' + helperService.getMonthNameShort(parseInt(blnTagihan[blnTagihan.length - 1]) - 1);
                    } else {
                        $scope.bulanTagihan = '';
                    }
                    ngDialog.open({
        ...
        var addEditController = function() {
            // console.log(date)
            $scope.month = helperService.month().options.sort(helperService.sortAsc);
            $scope.form = {
                no_kwitansi: '',
                tgl_kwitansi: '',
                nama_pembayar: '',
            ...
            var columnDetailActionTpl = '<div class="col-action">' + '<a href="" ng-click="grid.appScope.onDeleteDetailClick(row.entity)" >' + '<span class="badge bg-red"><i class="fa fa-trash"></i></span>' + '</a>' + '</div>';
            $scope.gridDetail = {
                enableMinHeightCheck: true,
                minRowsToShow: 5,
            ...
            function getRombelBySiswa(namaSiswa, callback) {
                ...
                cfpLoadingBar.start();
                return SiswaRombelService.getListBySiswa(paramdata).then(function(result) {
                    if (result.success) {
            ...
            function getById(id) {
                cfpLoadingBar.start();
                $resourceApi.getById(id).then(function(result) {
                    if (result.success) {
            ...
            function getkelas(params) {
                KelasService.getList(params).then(function(result) {
                    if (result.success) {
            ...
            $scope.onChangeDateTrans = function(sumber_kwitansi) {
                ...
                if (sumber_kwitansi == '1') {
                    ...
                    var tahunList = getTahunAjaranSelected($scope.form.tahun_ajaran_id);
                    $scope.form.year = (selectmonth >= 1 && selectmonth <= 6) ? (tahunList.tahun_akhir) : tahunList.tahun_awal;
                    getInfoTagihan();
                } else {
                    $scope.gridDetail.data = [{
                        index: 1,
            ...
            $scope.onSaveClick = function(event) {
                ...
                if ((typeof $scope.form.idrombel == 'undefined' || $scope.form.idrombel == null || $scope.form.idrombel == '') && (typeof $scope.form.kelasid == 'undefined' || $scope.form.kelasid == null || $scope.form.kelasid == '')) {
                ...
                if ($routeParams.id) {
                    ...
                    $resourceApi.insert(params).then(success, errorHandle);
                }
            }
